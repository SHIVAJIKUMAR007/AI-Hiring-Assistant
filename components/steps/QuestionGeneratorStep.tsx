...
const SkillChip: React.FC<{ skill: string }> = ({ skill }) => (
    <span className="inline-block bg-primary-100 dark:bg-primary-900/50 text-primary-800 dark:text-primary-300 text-xs font-medium mr-2 mb-2 px-2.5 py-0.5 rounded-full">
        {skill}
    </span>
);
...
const QuestionCard: React.FC<{ category: QuestionCategory }> = ({ category }) => (
    <Card>
        <h4 className="text-lg font-semibold text-primary-800 dark:text-primary-300 mb-3">{category.category} Questions</h4>
...
export const QuestionGeneratorStep: React.FC<QuestionGeneratorStepProps> = ({ roleAnalysis, onQuestionsGenerated, onBack }) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Interview Question Generator</h2>
      ...
      <Card>
        <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Role Analysis Summary</h3>
        <div className="mb-4">
            <h4 className="font-semibold text-gray-700 dark:text-gray-300 mb-2">Technical Skills</h4>
            <div>{roleAnalysis.technicalSkills.map(skill => <SkillChip key={skill} skill={skill} />)}</div>
        </div>
        <div>
            <h4 className="font-semibold text-gray-700 dark:text-gray-300 mb-2">Soft Skills</h4>
